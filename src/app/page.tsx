...
export default function Home() {
  ...
  // 팝업 방식을 우선 사용한다. redirect 방식은 accounts.google.com →
  // firebaseapp.com → 이 앱 이렇게 3개 도메인을 오가는데, 최신 Chrome의
  // 서드파티 저장소 분리(storage partitioning) 정책 때문에 돌아온 뒤
  // getRedirectResult가 에러 없이 그냥 null을 반환하며 로그인이 무효화되는
  // 문제가 실측 확인되었다. 팝업은 같은 탭 안에서 처리되어 이 문제를 피한다.
  // 팝업 자체가 막히는 환경(일부 인앱 브라우저 등)에서만 redirect로 폴백한다.
  async function handleGoogleSignIn() {
    setAuthError(null);
    try {
      await signInWithPopup(auth, googleProvider);
      addLog("signInWithPopup 성공");
    } catch (err) {
      const code = (err as { code?: string })?.code ?? "unknown";
      const message = err instanceof Error ? err.message : String(err);
      addLog(`signInWithPopup 실패: [${code}] ${message}`);

      if (code === "auth/popup-closed-by-user" || code === "auth/cancelled-popup-request") {
        // 사용자가 그냥 팝업을 닫은 것 - 에러로 취급하지 않는다.
        return;
      }
      ...
      if (
        ...
        addLog("팝업 사용 불가 환경 - redirect 방식으로 재시도");
        signInWithRedirect(auth, googleProvider).catch((redirectErr) => {
          ...
          setAuthError(`Google 로그인 시작 실패: ${redirectMessage}`);
        });
        return;
      }
