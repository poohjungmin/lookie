"use client";

import { Timestamp } from "firebase/firestore";
import { fetchHistoricalWeatherForLook } from "@/lib/weather";
import { updateLookWeatherFields, toDbWeather, type DbWeather } from "@/lib/lookStore";
import { resolveLookDate } from "@/lib/lookDate";

/**
 * 이미 저장된 룩의 촬영일(+ 있으면 GPS)로 Open-Meteo 과거 날씨를 다시
 * 조회해 weather/weatherStatus 필드만 덮어쓴다. 사진 원본·누끼·EXIF·lookId
 * 등은 전혀 다시 처리하지 않는다 - 상세 화면 "다시 조회" 버튼과 전체 룩의
 * "날씨 일괄 재조회"(bulkWeatherRecovery.ts)가 이 함수와 같은 하위 함수
 * (resolveLookDate/fetchHistoricalWeatherForLook/toDbWeather)를 공유한다.
 *
 * GPS가 없어도 실패로 취급하지 않는다 - fetchHistoricalWeatherForLook이
 * "실제 좌표 있으면 그걸로, 없으면 서울로"를 처리하므로, GPS 없이 실패
 * 상태로 저장됐던 예전 룩도 사진 재업로드나 위치 직접 입력 없이 복구된다.
 *
 * forceRefresh: true를 넘겨 weather.ts의 캐시를 무시하고 항상 새 요청을
 * 보낸다 - 사용자가 명시적으로 재시도를 누른 경로이므로 캐시에 남아있을
 * 수 있는 예전 결과를 절대 재사용하지 않는다(일괄 재조회는 반대로 이
 * 캐시를 적극 재사용해야 하므로 forceRefresh를 쓰지 않는다).
 *
 * 촬영일이 없으면 애초에 조회할 날짜가 없어 조회가 불가능하므로 호출부가
 * 먼저 걸러야 한다(상세 화면에서 버튼 자체를 숨김) - 이 함수는 그 가정을
 * 다시 한번 방어적으로 검사만 하고 그대로 throw한다. 날짜 판별은
 * resolveLookDate()로 하는데, 상세 화면 상단의 촬영일 표시와 완전히 같은
 * 기준이다.
 *
 * 성공 시 저장한 DbWeather를 그대로 반환한다 - 호출부가 Firestore를 다시
 * 읽지 않고도 로컬 캐시/화면을 즉시 갱신할 수 있게 하기 위함.
 */
export async function regenerateLookWeather(
  uid: string,
  look: { id: string; takenAt: Timestamp | null; latitude: number | null; longitude: number | null }
): Promise<DbWeather> {
  const takenAtDate = resolveLookDate(look);
  if (!takenAtDate) {
    throw new Error("촬영일 정보가 없어 날씨를 조회할 수 없어요");
  }

  const { result, locationSource } = await fetchHistoricalWeatherForLook(
    look.latitude,
    look.longitude,
    takenAtDate,
    {
      forceRefresh: true,
      logContext: { lookId: look.id, takenAt: takenAtDate.toISOString() },
    }
  );

  const weather = toDbWeather(result, locationSource);
  await updateLookWeatherFields(uid, look.id, { weather, weatherStatus: "success" });
  return weather;
}
